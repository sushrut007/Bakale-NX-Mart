      ...
      <div className="container mx-auto px-4 md:px-6 py-10">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-5">
            <ShoppingBag size={72} style={{ color: "var(--brand-maroon)", opacity: 0.15 }} />
            <h2 className="text-2xl font-black" style={{ color: "var(--brand-maroon)" }}>
              Your cart is empty
            </h2>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex justify-between items-center mb-2">
                <span className="font-black text-sm uppercase tracking-wider" style={{ color: "var(--brand-maroon)" }}>
                  {totalItems} Items
                </span>
                <button
                  onClick={clearCart}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: "var(--brand-warm-grey)" }}>
                        {item.product.brand} · {item.product.category}
                      </p>
                      <Link href={`/product/${item.product.id}`}>
                        <h3 className="font-black text-base leading-snug mb-2" style={{ color: "var(--brand-maroon)" }}>
                          {item.product.name}
                        </h3>
                      </Link>
                      ...
                      {item.selectedSize && (
                        <span
                          ...
                          style={{ backgroundColor: "var(--brand-blush)", color: "var(--brand-maroon)" }}
                        >
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-3">
                        ...
                        <span className="font-black text-xl" style={{ color: "var(--brand-red)" }}>
            ...
            <div className="lg:col-span-1">
              ...
              >
                <h2 className="font-black text-lg mb-5 uppercase tracking-wider" style={{ color: "var(--brand-maroon)" }}>
                  Order Summary
                </h2>

                <div className="space-y-3 mb-5">
                  <div className="flex justify-between text-sm">
                    <span style={{ color: "var(--brand-warm-grey)" }}>Subtotal ({totalItems} items)</span>
                    <span className="font-bold" style={{ color: "var(--brand-maroon)" }}>
                  ...
                  {shipping > 0 && (
                    <p className="text-xs" style={{ color: "var(--brand-warm-grey)" }}>
                      Add ₹{(1500 - totalPrice).toLocaleString("en-IN")} more for free shipping
                    </p>
                  )}
                  <div
                    className="h-px my-2"
                    style={{ backgroundColor: "rgba(59,10,10,0.1)" }}
                  />
                  <div className="flex justify-between">
                    <span className="font-black" style={{ color: "var(--brand-maroon)" }}>Total</span>
                    <span className="font-black text-2xl" style={{ color: "var(--brand-maroon)" }}>
                ...
                <div
                  ...
                  style={{ borderColor: "rgba(59,10,10,0.1)" }}
                >
                  {["🔒 Secure checkout", "📞 WhatsApp support", "✅ Quality guaranteed"].map(
                    (badge) => (
                      <p key={badge} className="text-xs" style={{ color: "var(--brand-warm-grey)" }}>
                        {badge}
                      </p>
                    )
                  )}
